...
def project_image(
    filepath: str, Phi: np.ndarray, num_dim: int, start_ef: int, img_dim: tuple
) -> np.ndarray:
...
def images_to_A(
    ...
    """Load + process images and stack in matrix as column vectors.
    ...
    for _, person_name in people.items():
...
def plot_projections(
    ...
    if num_dim == 1:
        ...
        ax.scatter(
            PER_1[0, :], PER_1[1, :], PER_1[2, :], c="r", label=people["person_1"]
        )
        ax.scatter(
            PER_2[0, :], PER_2[1, :], PER_2[2, :], c="b", label=people["person_2"]
        )
        ax.set_xlabel(f"eigenface {start_ef + 1}")
        ax.set_ylabel(f"eigenface {start_ef + 2}")
